...
const ORDER_TRANSITIONS: Record<(typeof ORDER_STATUSES)[number], string[]> = {
  ...
  preparing: ["courier", "shipped", "cancelled"],
  courier: ["shipped", "failed_delivery", "cancelled"],
  shipped: ["delivered", "failed_delivery", "returned"],
  delivered: ["returned", "refunded"],
  failed_delivery: ["courier", "returned", "cancelled"],
  cancelled: [],
  returned: ["refunded"],
  refunded: [],
};
...
function cleanArray(value: unknown, limit = 20, itemLimit = 80): string[] {
  const raw = Array.isArray(value)
    ...
    : typeof value === "string"
      ...
      : [];
  return [
    ...new Set(raw.map(item => clean(item, itemLimit)).filter(Boolean)),
  ].slice(0, limit);
}
...
const PROJECT_IMAGE_PREFIX =
  "https://bayzed123.github.io/zayaans-signature/images/catalogue/";
...
export function assertProjectOwnedProductMedia(
  imageUrl: string,
  gallery: string[]
): void {
  if (![imageUrl, ...gallery].every(isProjectOwnedImageUrl)) {
    throw new Error(
      "Use an approved project-owned image from the catalogue asset library"
    );
  }
}
...
function mapProduct(row: ProductRow) {
  return {
    ...
    sizes: parseArray(row.sizes_json),
    colours: parseArray(row.colours_json),
    imageUrl: row.image_url,
    gallery: parseArray(row.gallery_json),
    priceMinor: row.price_minor,
    compareAtMinor: row.compare_at_minor,
    vatNote: row.vat_note,
    fitInfo: row.fit_info,
    washCare: row.wash_care,
    availabilityNote: row.availability_note,
    tryOnEnabled: Boolean(row.try_on_enabled),
    brand: row.brand ?? "",
...
async function hasAdminSession(request: Request, env: Env): Promise<boolean> {
  ...
  if (
    ...
    !payload ||
    !signature ||
    signature !== (await tokenSignature(payload, secret))
  )
    return false;
  try {
...
export function adminCategoryWrite(body: Payload, existing?: CategoryRow) {
  ...
  if (!Number.isInteger(sortOrder) || sortOrder < 0 || sortOrder > 100000)
    throw new Error("Category order must be a whole number from 0 to 100000");
  if (!["women", "kids"].includes(audience))
    throw new Error("Category audience must be women or kids");
...
async function adminProductWrite(
  env: Env,
  body: Payload,
  existing?: ProductRow
) {
  ...
  if (!name || !Number.isInteger(priceMinor) || priceMinor < 0)
    throw new Error("A product name and valid price are required");
  const slug =
    slugify(clean(body.slug ?? existing?.slug ?? name, 120)) ||
    `piece-${Date.now()}`;
  const sku =
    clean(body.sku ?? existing?.sku, 60) ||
    `ZS-${Date.now().toString(36).toUpperCase()}`;
  const categoryId =
    body.categoryId === null || body.categoryId === ""
      ? null
      : Number(body.categoryId ?? existing?.category_id ?? 0) || null;
  const compareAtMinor = Number(
    body.compareAtMinor ?? existing?.compare_at_minor ?? 0
  );
  ...
  if (
    ...
    throw new Error("Price and stock values must be valid whole amounts");
  const status = clean(body.status ?? existing?.status ?? "draft", 20);
  if (!["draft", "active", "archived"].includes(status))
    throw new Error("Invalid product status");
  const imageUrl = clean(body.imageUrl ?? existing?.image_url, 500);
  const galleryItems = cleanArray(
    body.gallery ?? parseArray(existing?.gallery_json ?? "[]"),
    12,
    500
  );
  ...
  return {
    ...
    stock,
    status,
    summary: clean(body.summary ?? existing?.summary, 300),
    description: clean(body.description ?? existing?.description, 5000),
    fabric: clean(body.fabric ?? existing?.fabric, 120),
    leadTime: clean(body.leadTime ?? existing?.lead_time, 120),
    sizeGuide: clean(body.sizeGuide ?? existing?.size_guide, 1200),
    sizes: JSON.stringify(
      cleanArray(body.sizes ?? parseArray(existing?.sizes_json ?? "[]"))
    ),
    colours: JSON.stringify(
      cleanArray(body.colours ?? parseArray(existing?.colours_json ?? "[]"))
    ),
    imageUrl,
    ...
    featured:
      body.featured === undefined
        ...
        : body.featured
          ? 1
          : 0,
    brand: clean(body.brand ?? existing?.brand, 80),
    isNewArrival:
      body.isNewArrival === undefined
        ? (existing?.is_new_arrival ?? 0)
        : body.isNewArrival
          ? 1
          : 0,
    isOffer:
    ...
    isBestSeller:
      body.isBestSeller === undefined
        ? (existing?.is_best_seller ?? 0)
        : body.isBestSeller
          ? 1
          : 0,
    vatNote:
      clean(body.vatNote ?? existing?.vat_note ?? "+ VAT", 80) || "+ VAT",
    fitInfo: clean(body.fitInfo ?? existing?.fit_info, 500),
    washCare: clean(body.washCare ?? existing?.wash_care, 1000),
    availabilityNote: clean(
      body.availabilityNote ?? existing?.availability_note,
      300
    ),
    tryOnEnabled:
      body.tryOnEnabled === undefined
        ? (existing?.try_on_enabled ?? 1)
        : body.tryOnEnabled
          ? 1
          : 0,
  };
...
async function publicCatalogue(request: Request, env: Env) {
  ...
  if (brand) {
    ...
    bindings.push(brand.toLowerCase());
  }
  ...
  if (["new", "offer", "best_seller"].includes(promotion))
    where.push(
      promotion === "new"
        ? "p.is_new_arrival = 1"
        : promotion === "offer"
  ...
  const orderBy: Record<string, string> = {
    ...
    price_asc: "p.price_minor ASC, p.name COLLATE NOCASE",
    price_desc: "p.price_minor DESC, p.name COLLATE NOCASE",
    name_asc: "p.name COLLATE NOCASE",
  };
  const { results } = await env.COMMERCE.prepare(
    ...
    .bind(...bindings)
    .all<ProductRow>();
  const { results: brands } = await env.COMMERCE.prepare(
    "SELECT DISTINCT brand FROM products WHERE status = 'active' AND brand != '' ORDER BY brand COLLATE NOCASE"
  ).all<{ brand: string }>();
  return json(
    {
      products: (results ?? []).map(mapProduct),
      categories: await categories(env),
      filters: { brands: (brands ?? []).map(row => row.brand) },
    },
    200,
    request,
    env
...
async function createOrder(request: Request, env: Env) {
  ...
  const normalized = items
    .map(raw => (raw && typeof raw === "object" ? (raw as Payload) : null))
    .filter(Boolean) as Payload[];
  const quantities = normalized.map(item => ({
    id: Number(item.productId),
    qty: Number(item.qty),
    size: clean(item.size, 60),
    colour: clean(item.colour, 60),
  }));
  if (
    quantities.some(
      item =>
    ...
    return json(
      { error: "One or more cart quantities are invalid" },
      422,
      request,
      env
    );
  ...
  for (const item of quantities)
    requestedByProduct.set(
      item.id,
      (requestedByProduct.get(item.id) ?? 0) + item.qty
    );
  if (
    [...requestedByProduct].some(([id, qty]) => products.get(id)!.stock < qty)
  )
    return json(
      { error: "A selected piece no longer has sufficient stock" },
      409,
      request,
      env
    );
  const lineItems = quantities.map(item => ({
    ...item,
  ...
  for (const item of lineItems) {
    statements.push(
      env.COMMERCE.prepare(
        "INSERT INTO order_items (order_id, product_id, sku, name, image_url, size, colour, qty, unit_price_minor, line_total_minor) VALUES ((SELECT id FROM orders WHERE order_no = ?), ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ).bind(
        orderNo,
        item.product.id,
        item.product.sku,
        ...
        item.product.price_minor * item.qty
      )
    );
    statements.push(
      env.COMMERCE.prepare(
  ...
  statements.push(
    env.COMMERCE.prepare(
      "INSERT INTO order_events (order_id, status, note) VALUES ((SELECT id FROM orders WHERE order_no = ?), 'pending', 'Order received')"
    ).bind(orderNo)
  );
  try {
    ...
    return json(
      {
        error:
          "This order could not be placed. Please refresh your cart and try again.",
      },
      409,
      request,
      env
...
async function trackOrder(request: Request, env: Env) {
  ...
  const order = await env.COMMERCE.prepare(
    ...
    .first();
  if (!order)
    return json({ error: "No matching order was found" }, 404, request, env);
  const { results: items } = await env.COMMERCE.prepare(
    "SELECT name, image_url, size, colour, qty, unit_price_minor, line_total_minor FROM order_items WHERE order_id = ?"
  )
    .bind(order.id)
    .all();
  const { results: events } = await env.COMMERCE.prepare(
...
async function adminRoute(
  ...
  if (url.pathname === "/api/admin/session" && request.method === "POST") {
    if (!env.ADMIN_PASSWORD || !env.ADMIN_USERNAME)
      return json(
        { error: "Administrator access is not configured yet" },
        503,
        request,
        env
      );
    ...
    return json(
      { token: await issueAdminToken(env.ADMIN_PASSWORD), username },
      200,
      request,
      env
    );
  ...
  if (!(await hasAdminSession(request, env)))
    return json({ error: "Administrator session required" }, 401, request, env);
  if (url.pathname === "/api/admin/overview" && request.method === "GET") {
    const [products, orders, lowStock] = await env.COMMERCE.batch([
      env.COMMERCE.prepare(
        "SELECT COUNT(*) AS count FROM products WHERE status != 'archived'"
      ),
      env.COMMERCE.prepare(
        "SELECT COUNT(*) AS count FROM orders WHERE status IN ('pending','confirmed','preparing')"
      ),
      env.COMMERCE.prepare(
        "SELECT COUNT(*) AS count FROM products WHERE status = 'active' AND stock <= low_stock_threshold"
      ),
    ]);
    const productCountRow = products.results?.[0] as
      | { count?: number }
      | undefined;
    const openOrdersRow = orders.results?.[0] as { count?: number } | undefined;
    const lowStockRow = lowStock.results?.[0] as { count?: number } | undefined;
    return json(
      {
  ...
  if (
    url.pathname === "/api/admin/business-overview" &&
    request.method === "GET"
  ) {
    const [
      sales,
      ...
      env.COMMERCE.prepare(
        "SELECT COALESCE(SUM(total_minor), 0) AS total_minor, COUNT(*) AS count FROM orders WHERE status NOT IN ('cancelled','refunded')"
      ),
      ...
      env.COMMERCE.prepare(
        "SELECT COUNT(*) AS active_products, COALESCE(SUM(stock), 0) AS units_on_hand, SUM(CASE WHEN stock <= low_stock_threshold THEN 1 ELSE 0 END) AS low_stock FROM products WHERE status = 'active'"
      ),
      env.COMMERCE.prepare(
        "SELECT status, COUNT(*) AS count FROM orders GROUP BY status ORDER BY count DESC, status"
      ),
      env.COMMERCE.prepare(
        "SELECT substr(created_at, 1, 10) AS day, COALESCE(SUM(total_minor), 0) AS total_minor, COUNT(*) AS order_count FROM orders WHERE status NOT IN ('cancelled','refunded') GROUP BY substr(created_at, 1, 10) ORDER BY day DESC LIMIT 7"
      ),
      env.COMMERCE.prepare(
        "SELECT oi.product_id, MAX(oi.name) AS name, SUM(oi.qty) AS units_sold, COALESCE(SUM(oi.line_total_minor), 0) AS sales_minor FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE o.status NOT IN ('cancelled','refunded') GROUP BY oi.product_id ORDER BY units_sold DESC, sales_minor DESC LIMIT 5"
      ),
    ]);
    const salesRow = sales.results?.[0] as
      | { total_minor?: number; count?: number }
      | undefined;
    const orderCountRow = allOrders.results?.[0] as
      | { count?: number }
      | undefined;
    const customerCountRow = customers.results?.[0] as
      | { count?: number }
      | undefined;
    return json(
      {
        sales: {
          totalMinor: salesRow?.total_minor ?? 0,
          orderCount: salesRow?.count ?? 0,
        },
        ...
        dailySales: [...(dailySales.results ?? [])].reverse(),
  ...
  if (
    url.pathname === "/api/admin/courier-workspace" &&
    request.method === "GET"
  ) {
    const [ready, active, exceptions, states] = await env.COMMERCE.batch([
      env.COMMERCE.prepare(
        "SELECT id, order_no, customer_name, customer_phone, status, total_minor, courier_consignment_id, courier_tracking_code, courier_status, created_at FROM orders WHERE status IN ('confirmed','preparing','courier','shipped') AND (courier_consignment_id IS NULL OR courier_tracking_code IS NULL) ORDER BY id DESC LIMIT 50"
      ),
      env.COMMERCE.prepare(
        "SELECT id, order_no, customer_name, customer_phone, status, total_minor, courier_consignment_id, courier_tracking_code, courier_status, created_at FROM orders WHERE courier_consignment_id IS NOT NULL OR courier_tracking_code IS NOT NULL ORDER BY id DESC LIMIT 100"
      ),
      env.COMMERCE.prepare(
        "SELECT id, order_no, customer_name, customer_phone, status, total_minor, courier_consignment_id, courier_tracking_code, courier_status, created_at FROM orders WHERE status IN ('failed_delivery','returned','cancelled') ORDER BY id DESC LIMIT 50"
      ),
      env.COMMERCE.prepare(
        "SELECT COALESCE(courier_status, 'not_dispatched') AS status, COUNT(*) AS count FROM orders GROUP BY COALESCE(courier_status, 'not_dispatched') ORDER BY count DESC, status"
      ),
    ]);
    return json(
      {
        dispatchReady: ready.results ?? [],
        activeConsignments: active.results ?? [],
        exceptions: exceptions.results ?? [],
        statusCounts: states.results ?? [],
        provider: {
          name: "Steadfast Courier",
          liveDispatchReady: false,
          note: "Live dispatch remains blocked until the Steadfast account is activated.",
        },
      },
      200,
      request,
  ...
  if (url.pathname === "/api/admin/categories" && request.method === "GET")
    return json({ categories: await categories(env, true) }, 200, request, env);
  if (url.pathname === "/api/admin/categories" && request.method === "POST") {
    const body = await readPayload(request);
    if (!body)
      return json({ error: "Invalid category payload" }, 400, request, env);
    try {
      const category = adminCategoryWrite(body);
      const inserted = await env.COMMERCE.prepare(
        "INSERT INTO categories (name, slug, description, image_url, sort_order, parent_label, audience, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING id"
      )
        .bind(
          category.name,
          category.slug,
        ...
        .first<{ id: number }>();
      return json({ id: inserted?.id, ...category }, 201, request, env);
    } catch (error) {
      return json(
        {
          error:
  ...
  if (categoryMatch && request.method === "PATCH") {
    ...
    if (!body)
      return json({ error: "Invalid category payload" }, 400, request, env);
    const current = await env.COMMERCE.prepare(
      "SELECT * FROM categories WHERE id = ?"
    )
      .bind(id)
      .first<CategoryRow>();
    if (!current)
      return json({ error: "Category not found" }, 404, request, env);
    try {
      const category = adminCategoryWrite(body, current);
      await env.COMMERCE.prepare(
        "UPDATE categories SET slug=?, name=?, description=?, image_url=?, sort_order=?, parent_label=?, audience=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?"
      )
        .bind(
          category.slug,
          category.name,
        ...
        .run();
      return json({ id, ...category }, 200, request, env);
    } catch (error) {
      return json(
        {
          error:
  ...
  if (categoryMatch && request.method === "DELETE") {
    ...
    if (!current)
      return json({ error: "Category not found" }, 404, request, env);
    const references = await env.COMMERCE.prepare(
    ...
    if (Number(references?.count ?? 0) > 0)
      return json(
        {
          error:
            "This category contains products and cannot be deleted. Archive it or reassign its products first.",
        },
        409,
        request,
        env
      );
    await env.COMMERCE.prepare("DELETE FROM categories WHERE id = ?")
      .bind(id)
      .run();
  ...
  if (url.pathname === "/api/admin/inventory" && request.method === "GET") {
    const [inventory, adjustments] = await env.COMMERCE.batch([
      env.COMMERCE.prepare(
        "SELECT p.id, p.name, p.sku, p.stock, p.low_stock_threshold, p.status, COALESCE(SUM(CASE WHEN o.status != 'cancelled' THEN oi.qty ELSE 0 END), 0) AS sold_qty FROM products p LEFT JOIN order_items oi ON oi.product_id = p.id LEFT JOIN orders o ON o.id = oi.order_id GROUP BY p.id ORDER BY CASE WHEN p.stock = 0 THEN 0 WHEN p.stock <= p.low_stock_threshold THEN 1 ELSE 2 END, p.stock, p.name LIMIT 250"
      ),
      env.COMMERCE.prepare(
        "SELECT ia.id, ia.product_id, p.name AS product_name, ia.previous_stock, ia.quantity_delta, ia.resulting_stock, ia.reason, ia.note, ia.created_at FROM inventory_adjustments ia JOIN products p ON p.id = ia.product_id ORDER BY ia.id DESC LIMIT 100"
      ),
    ]);
    return json(
      {
  ...
  const inventoryMatch = url.pathname.match(
    /^\/api\/admin\/products\/(\d+)\/inventory$/
  );
  if (inventoryMatch && request.method === "POST") {
    ...
    if (
      ...
      return json(
        { error: "Provide a non-zero whole-unit adjustment and reason" },
        422,
        request,
        env
      );
    if (
      ...
      return json(
        { error: "Low-stock threshold is invalid" },
        422,
        request,
        env
      );
    const product = await env.COMMERCE.prepare(
      "SELECT id, stock, low_stock_threshold FROM products WHERE id = ?"
    )
      .bind(id)
      .first<{ id: number; stock: number; low_stock_threshold: number }>();
    if (!product)
      return json({ error: "Product not found" }, 404, request, env);
    const resulting = product.stock + delta;
    if (resulting < 0)
      return json(
        { error: "This adjustment would make stock negative" },
        422,
        request,
        env
      );
    const nextThreshold = Number.isFinite(threshold)
      ? threshold
      : product.low_stock_threshold;
    await env.COMMERCE.batch([
      env.COMMERCE.prepare(
        "UPDATE products SET stock = ?, low_stock_threshold = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
      ).bind(resulting, nextThreshold, id),
      env.COMMERCE.prepare(
        "INSERT INTO inventory_adjustments (product_id, previous_stock, quantity_delta, resulting_stock, reason, note) VALUES (?, ?, ?, ?, ?, ?)"
      ).bind(id, product.stock, delta, resulting, reason, note),
    ]);
    return json(
      {
        id,
  ...
  if (url.pathname === "/api/admin/products" && request.method === "POST") {
    const body = await readPayload(request);
    if (!body)
      return json({ error: "Invalid product payload" }, 400, request, env);
    try {
      const product = await adminProductWrite(env, body);
      const row = await env.COMMERCE.prepare(
        "INSERT INTO products (slug, name, sku, category_id, summary, description, fabric, lead_time, size_guide, sizes_json, colours_json, image_url, gallery_json, price_minor, compare_at_minor, stock, status, featured, brand, is_new_arrival, is_offer, is_best_seller, vat_note, fit_info, wash_care, availability_note, try_on_enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING id"
      )
        .bind(
          product.slug,
          product.name,
  ...
  if (productMatch && request.method === "PATCH") {
    ...
    try {
      ...
      await env.COMMERCE.prepare(
        "UPDATE products SET slug=?, name=?, sku=?, category_id=?, summary=?, description=?, fabric=?, lead_time=?, size_guide=?, sizes_json=?, colours_json=?, image_url=?, gallery_json=?, price_minor=?, compare_at_minor=?, stock=?, status=?, featured=?, brand=?, is_new_arrival=?, is_offer=?, is_best_seller=?, vat_note=?, fit_info=?, wash_care=?, availability_note=?, try_on_enabled=?, updated_at=CURRENT_TIMESTAMP WHERE id=?"
      )
        .bind(
          product.slug,
          product.name,
  ...
  if (productMatch && request.method === "DELETE") {
    ...
    if (Number(references?.count ?? 0) > 0)
      return json(
        {
          error:
            "This product is referenced by an order and cannot be deleted. Archive it instead.",
        },
        409,
        request,
        env
      );
    await env.COMMERCE.prepare("DELETE FROM products WHERE id = ?")
      .bind(id)
      .run();
    return json({ id, deleted: true }, 200, request, env);
  }
  if (url.pathname === "/api/admin/orders" && request.method === "GET") {
    const { results } = await env.COMMERCE.prepare(
      "SELECT id, order_no, customer_name, customer_phone, status, total_minor, created_at, courier_consignment_id, courier_tracking_code, courier_status FROM orders ORDER BY id DESC LIMIT 200"
    ).all();
  ...
  if (orderTimelineMatch && request.method === "GET") {
    ...
    const { results } = await env.COMMERCE.prepare(
      "SELECT id, status, note, created_at FROM order_events WHERE order_id = ? ORDER BY id DESC LIMIT 100"
    )
      .bind(id)
      .all();
    return json({ orderId: id, events: results ?? [] }, 200, request, env);
  }
  const courierCreateMatch = url.pathname.match(
    /^\/api\/admin\/orders\/(\d+)\/courier$/
  );
  if (courierCreateMatch && request.method === "POST") {
    const id = Number(courierCreateMatch[1]);
    const order = await env.COMMERCE.prepare(
      "SELECT id, order_no, customer_name, customer_phone, address, note, total_minor, courier_consignment_id, courier_tracking_code, courier_status FROM orders WHERE id = ?"
    )
      .bind(id)
      .first<{
        id: number;
    ...
    if (!order) return json({ error: "Order not found" }, 404, request, env);
    if (order.courier_consignment_id || order.courier_tracking_code)
      return json(
        { error: "A Steadfast consignment already exists for this order" },
        409,
        request,
        env
      );
    const recipientPhone = phoneDigits(order.customer_phone);
    if (!recipientPhone || !order.customer_name || !order.address)
      return json(
        {
          error:
            "This order is missing recipient details required by Steadfast",
        },
        422,
        request,
        env
    ...
    try {
      const courier = await steadfastRequest<SteadfastCreateResponse>(
        env,
        "/create_order",
        "POST",
        {
      ...
      const consignmentId =
        courier.consignment?.consignment_id === undefined ||
        courier.consignment.consignment_id === null
          ? ""
          : String(courier.consignment.consignment_id);
      ...
      await env.COMMERCE.batch([
        env.COMMERCE.prepare(
          "UPDATE orders SET courier_consignment_id = ?, courier_tracking_code = ?, courier_status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
        ).bind(consignmentId, trackingCode, courierStatus || "in_review", id),
        env.COMMERCE.prepare(
          "INSERT INTO order_events (order_id, status, note) VALUES (?, ?, ?)"
        ).bind(id, "courier", `Steadfast consignment created: ${trackingCode}`),
      ]);
      return json(
        {
          id,
      ...
      return json(
        {
          error:
            error instanceof Error
              ? error.message
              : "Unable to create the Steadfast consignment",
        },
        502,
        request,
        env
  ...
  const courierStatusMatch = url.pathname.match(
    /^\/api\/admin\/orders\/(\d+)\/courier-status$/
  );
  if (courierStatusMatch && request.method === "GET") {
    const id = Number(courierStatusMatch[1]);
    const order = await env.COMMERCE.prepare(
      "SELECT id, order_no, courier_consignment_id, courier_tracking_code FROM orders WHERE id = ?"
    )
      .bind(id)
      .first<{
        id: number;
    ...
    if (!order.courier_consignment_id && !order.courier_tracking_code)
      return json(
        {
          error:
            "Create a Steadfast consignment before refreshing delivery status",
        },
        409,
        request,
        env
    ...
    try {
      const courier = await steadfastRequest<SteadfastStatusResponse>(
        env,
        `/status_by_invoice/${encodeURIComponent(order.order_no)}`,
        "GET"
      );
      ...
      await env.COMMERCE.prepare(
        "UPDATE orders SET courier_status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
      )
        .bind(courierStatus, id)
        .run();
      ...
      return json(
        {
          error:
            error instanceof Error
              ? error.message
              : "Unable to refresh the Steadfast delivery status",
        },
        502,
        request,
        env
  ...
  if (orderMatch && request.method === "PATCH") {
    ...
    if (!ORDER_STATUSES.includes(status as (typeof ORDER_STATUSES)[number]))
      return json({ error: "Invalid order status" }, 422, request, env);
    const order = await env.COMMERCE.prepare(
    ...
    if (
      status !== order.status &&
      !ORDER_TRANSITIONS[
        order.status as keyof typeof ORDER_TRANSITIONS
      ]?.includes(status)
    )
      return json(
    ...
    const statements = [
      ...
      env.COMMERCE.prepare(
        "INSERT INTO order_events (order_id, status, note) VALUES (?, ?, ?)"
      ).bind(id, status, clean(body?.note, 500)),
    ];
    if (status === "cancelled" && order.status !== "cancelled")
      statements.push(
        env.COMMERCE.prepare(
          "UPDATE products SET stock = stock + (SELECT qty FROM order_items WHERE order_id = ? AND product_id = products.id) WHERE id IN (SELECT product_id FROM order_items WHERE order_id = ? AND product_id IS NOT NULL)"
        ).bind(id, id)
      );
    await env.COMMERCE.batch(statements);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/api/health" && request.method === "GET")
      return json(
        { status: "ok", service: "zayaans-signature-api", commerce: true },
        200,
        request,
        env
      );
    if (url.pathname === "/api/newsletter" && request.method === "POST") {
      ...
      await env.NEWSLETTER.prepare(
        "INSERT INTO newsletter_subscribers (email, source, consent_at) VALUES (?, 'website', CURRENT_TIMESTAMP) ON CONFLICT(email) DO UPDATE SET source = excluded.source, consent_at = excluded.consent_at"
      )
        .bind(email)
        .run();
      return json({ success: true }, 201, request, env);
    ...
    if (url.pathname === "/api/categories" && request.method === "GET")
      return json({ categories: await categories(env) }, 200, request, env);
    if (url.pathname === "/api/products" && request.method === "GET")
      return publicCatalogue(request, env);
    const productMatch = url.pathname.match(/^\/api\/products\/([a-z0-9-]+)$/);
    if (productMatch && request.method === "GET") {
      const row = await env.COMMERCE.prepare(
        "SELECT p.*, c.name AS category_name, c.slug AS category_slug FROM products p LEFT JOIN categories c ON c.id=p.category_id WHERE p.slug=? AND p.status='active'"
      )
        .bind(productMatch[1])
        .first<ProductRow>();
      return row
        ? json({ product: mapProduct(row) }, 200, request, env)
        : json({ error: "Piece not found" }, 404, request, env);
    }
