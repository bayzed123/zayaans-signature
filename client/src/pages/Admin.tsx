...
import { BarChart3, Bell, Boxes, ChevronRight, ClipboardList, Link2, LockKeyhole, Menu, PackagePlus, PencilLine, Plus, RefreshCw, Search, Tags, Trash2, Truck, UserRound, X } from "lucide-react";
...
export default function Admin() {
  ...
  return <div className="min-h-screen bg-[#eee8dd] text-[#171512]"><div className="flex min-h-screen">
    ...
    <main className="min-w-0 flex-1 px-5 py-5 pb-8 sm:px-8 lg:px-12"><AdminShellHeader search={adminSearch} setSearch={setAdminSearch} products={products} orders={orders} lowStock={overview?.lowStock ?? 0} openOrders={overview?.openOrders ?? 0} onNavigate={selectTab} onMenu={() => setMobileMenuOpen(true)} profileOpen={profileMenuOpen} setProfileOpen={setProfileMenuOpen} onSignOut={() => { sessionStorage.removeItem(TOKEN_KEY); setToken(""); }} /><header className="mt-8 flex flex-wrap items-end justify-between gap-5 border-b border-black/12 pb-6"><div><p className="section-kicker">Atelier operations</p><h1 className="mt-3 font-display text-5xl">{activeTab?.label}</h1></div><div className="flex items-center gap-4"><Link href="/" className="font-ui text-[10px] font-bold uppercase tracking-[.15em] text-black/53 hover:text-[#8f6b2c]">View boutique</Link><button onClick={() => void refresh()} className="min-h-11 border border-black/15 px-3 py-2 font-ui text-[10px] font-bold uppercase tracking-[.14em] hover:border-[#8f6b2c]">Refresh</button></div></header>
...
export function AdminShellHeader({ search, setSearch, products, orders, lowStock, openOrders, onNavigate, onMenu, profileOpen, setProfileOpen, onSignOut }: { search: string; setSearch: (value: string) => void; products: Product[]; orders: AdminOrder[]; lowStock: number; openOrders: number; onNavigate: (tab: Tab) => void; onMenu: () => void; profileOpen: boolean; setProfileOpen: (value: boolean) => void; onSignOut: () => void }) {
  const term = search.trim().toLowerCase();
  const productMatches = term ? products.filter((product) => `${product.name} ${product.sku}`.toLowerCase().includes(term)).slice(0, 4) : [];
  const orderMatches = term ? orders.filter((order) => `${order.order_no} ${order.customer_name} ${order.customer_phone}`.toLowerCase().includes(term)).slice(0, 4) : [];
  const notificationCount = lowStock + openOrders;
  return <div className="relative flex min-h-14 items-center gap-3 border-b border-black/12 pb-5"><button type="button" aria-label="Open administrator navigation" aria-controls="private-admin-drawer" onClick={onMenu} className="grid h-11 w-11 shrink-0 place-items-center border border-black/15 text-black md:hidden"><Menu size={19} /></button><div className="hidden shrink-0 md:block"><p className="font-display text-2xl leading-none">Atelier <span className="text-[#8f6b2c]">Console</span></p><p className="mt-1 font-ui text-[8px] font-bold uppercase tracking-[.18em] text-black/45">Private operations</p></div><div className="relative mx-auto w-full max-w-xl"><label className="flex min-h-11 items-center gap-2 border border-black/15 bg-white/55 px-3 focus-within:border-[#8f6b2c]"><Search size={15} className="text-black/46" /><span className="sr-only">Search private catalogue or orders</span><input aria-label="Search private catalogue or orders" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products, SKU, orders or customers" className="w-full bg-transparent font-ui text-xs outline-none placeholder:text-black/38" /></label>{term && <div className="absolute z-40 mt-2 w-full border border-black/14 bg-[#f8f4ed] p-2 shadow-[0_16px_36px_rgba(23,21,18,.13)]"><p className="px-2 py-1 font-ui text-[8px] font-bold uppercase tracking-[.15em] text-black/45">Private search</p>{productMatches.map((product) => <button type="button" key={`product-${product.id}`} onClick={() => { onNavigate("products"); setSearch(""); }} className="flex w-full items-center justify-between gap-3 px-2 py-2.5 text-left hover:bg-[#ece4d7]"><span className="font-ui text-xs font-semibold">{product.name}</span><span className="font-ui text-[9px] uppercase tracking-[.1em] text-black/45">{product.sku}</span></button>)}{orderMatches.map((order) => <button type="button" key={`order-${order.id}`} onClick={() => { onNavigate("orders"); setSearch(""); }} className="flex w-full items-center justify-between gap-3 px-2 py-2.5 text-left hover:bg-[#ece4d7]"><span className="font-ui text-xs font-semibold">{order.order_no}</span><span className="font-ui text-[9px] uppercase tracking-[.1em] text-black/45">{order.customer_name}</span></button>)}{!productMatches.length && !orderMatches.length && <p className="px-2 py-3 font-ui text-xs text-black/52">No matching private records.</p>}</div>}</div><button type="button" onClick={() => onNavigate(lowStock ? "inventory" : "orders")} className="relative grid h-11 w-11 shrink-0 place-items-center border border-black/15 text-black hover:border-[#8f6b2c]" aria-label={`Operations notifications: ${notificationCount}`}><Bell size={17} />{notificationCount > 0 && <span className="absolute -right-1 -top-1 grid h-4 min-w-4 place-items-center rounded-full bg-[#8f6b2c] px-1 font-ui text-[8px] font-bold text-white">{notificationCount}</span>}</button><div className="relative"><button type="button" aria-expanded={profileOpen} aria-label="Open administrator profile menu" onClick={() => setProfileOpen(!profileOpen)} className="grid h-11 w-11 place-items-center bg-[#171512] text-[--gold]"><UserRound size={17} /></button>{profileOpen && <div className="absolute right-0 z-40 mt-2 w-48 border border-black/14 bg-[#f8f4ed] p-2 shadow-[0_16px_36px_rgba(23,21,18,.13)]"><p className="px-2 py-2 font-ui text-[9px] font-bold uppercase tracking-[.13em] text-black/48">Administrator session</p><Link href="/" className="block px-2 py-2.5 font-ui text-xs hover:bg-[#ece4d7]">View boutique</Link><button type="button" onClick={onSignOut} className="w-full px-2 py-2.5 text-left font-ui text-xs text-red-900 hover:bg-red-900 hover:text-white">Sign out</button></div>}</div></div>;
}

export function MobileAdminDrawer({ tabs, activeTab, selectTab, onClose, openProductEditor }: { tabs: Array<{ id: Tab; label: string; icon: typeof BarChart3 }>; activeTab: Tab; selectTab: (tab: Tab) => void; onClose: () => void; openProductEditor: () => void }) {
  return <div id="private-admin-drawer" className="fixed inset-0 z-[70] md:hidden" role="dialog" aria-modal="true" aria-label="Administrator navigation"><button type="button" aria-label="Close administrator navigation" onClick={onClose} className="absolute inset-0 bg-black/45" /><aside className="relative flex h-full w-[min(86vw,340px)] flex-col bg-[#13110f] p-6 text-white shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="font-display text-3xl">Zayaan&apos;s <span className="text-[--gold]">Signature</span></p><p className="mt-2 font-ui text-[8px] font-bold uppercase tracking-[.2em] text-white/42">Private operations</p></div><button type="button" aria-label="Close administrator navigation" onClick={onClose} className="grid h-10 w-10 place-items-center border border-white/15 text-white"><X size={17} /></button></div><button type="button" onClick={openProductEditor} className="mt-8 inline-flex min-h-12 items-center justify-center gap-2 bg-[--gold] px-4 font-ui text-[10px] font-bold uppercase tracking-[.15em] text-black"><PackagePlus size={15} /> Add product</button><nav aria-label="Administrator sections" className="mt-7 space-y-1">{tabs.map((item) => <button type="button" key={item.id} onClick={() => selectTab(item.id)} className={`flex min-h-12 w-full items-center gap-3 px-3 font-ui text-[10px] font-bold uppercase tracking-[.15em] ${activeTab === item.id ? "bg-[--gold] text-black" : "text-white/65 hover:bg-white/10 hover:text-white"}`}><item.icon size={16} />{item.label}</button>)}</nav><Link href="/" className="mt-auto font-ui text-[9px] font-bold uppercase tracking-[.14em] text-white/48 hover:text-[--gold]">View boutique</Link></aside></div>;
}
...
const orderStatuses = ["pending", "confirmed", "preparing", "courier", "shipped", "delivered", "failed_delivery", "cancelled", "returned", "refunded"];
...
function OrderTimeline({ order, token }: { order: AdminOrder; token: string }) {
  const [open, setOpen] = useState(false); const [events, setEvents] = useState<OrderTimelineEvent[] | null>(null); const [error, setError] = useState("");
  async function toggleTimeline() {
    if (open) { setOpen(false); return; }
    setOpen(true); setError("");
    if (events) return;
    try { const data = await adminRequest<{ events: OrderTimelineEvent[] }>(`/api/admin/orders/${order.id}/timeline`, token); setEvents(data.events); }
    catch { setError("Timeline is temporarily unavailable."); }
  }
  return <div className="mt-4"><button type="button" aria-expanded={open} onClick={() => void toggleTimeline()} className="font-ui text-[9px] font-bold uppercase tracking-[.13em] text-[#8f6b2c] underline-offset-4 hover:underline">{open ? "Hide lifecycle timeline" : "View lifecycle timeline"}</button>{open && <div className="mt-3 border-l border-[#8f6b2c]/55 pl-4"><p className="font-ui text-[8px] font-bold uppercase tracking-[.15em] text-black/45">Private lifecycle record</p>{!events && !error && <p className="mt-2 font-ui text-xs text-black/52">Loading timeline…</p>}{error && <p className="mt-2 font-ui text-xs text-red-900">{error}</p>}{events?.map((event) => <div key={event.id} className="relative mt-3"><span className="absolute -left-[21px] top-1.5 h-2 w-2 rounded-full bg-[#8f6b2c]" /><p className="font-ui text-[10px] font-bold uppercase tracking-[.12em] text-black">{event.status.replace(/_/g, " ")}</p><p className="mt-1 font-ui text-xs leading-5 text-black/58">{event.note || "No internal note added."}</p><time className="mt-1 block font-ui text-[9px] uppercase tracking-[.1em] text-black/42">{new Date(event.created_at).toLocaleString()}</time></div>)}{events?.length === 0 && <p className="mt-2 font-ui text-xs text-black/52">No lifecycle events are recorded yet.</p>}</div>}</div>;
}
